...
const CreateAbsentForm = ({ afterSubmit, onCancel, journal }) => {
    ...
    const addAbsent = async (formData) => {
        try {
            await axiosInstance.post("/student-absent", formData, {
                headers: {
                    jwt: getToken(authHeader)
                }
            });
            toast.success(MESSAGES.studentAbsents.addSuccess);
            afterSubmit();
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message;
            console.log(`${MESSAGES.studentAbsents.addError}: `, errorMessage);
            if (error.status !== 500) {
        ...
        <div>
            {
                ...
                <form
                    ...
                    onSubmit={handleSubmit((formData) => {
                        addAbsent(transformFormData(formData, journal?.id));
                    })}
                >
...
export default CreateAbsentForm;
